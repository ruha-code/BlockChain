const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      },
      {
        id: "transfer",
        label: "Transfer",
        icon: <><path d="M5 12h14" /><path d="M14 6l6 6-6 6" /></>,
        accent: "blue",
      },
      {
        id: "transactions",
...
function NavItem({ item, active, onClick }) {
  ...
  return (
    ...
    >
      {/* Active indicator */}
      {active && (
        <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-white/50 rounded-full" />
      )}
...
export default function Sidebar({ activeTab, setActiveTab, isOwner, onDisconnect, txCount, username, isRegistered }) {
  return (
    <aside className="w-[220px] bg-white border-r border-gray-100 flex flex-col fixed h-screen z-[100]">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
        {NAV_GROUPS.map((group) => (
          <div key={group.label}>
            <p className="text-[10px] font-bold text-gray-300 uppercase tracking-widest px-3 mb-1.5">{group.label}</p>
